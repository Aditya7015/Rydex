...
import { FaCarSide, FaGasPump, FaRoute, FaCar, FaLocationArrow } from 'react-icons/fa';
...
export default function MyRides() {
  ...
  const getMinutesUntilRide = (ride) => {
    const rideDateTime = getRideDateTime(ride);
    if (!rideDateTime) return Infinity;
    return (rideDateTime - new Date()) / (1000 * 60);
  };
        ...
        {filteredRides.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredRides.map((ride) => {
              ...
              const canStartTracking = ride.status === 'active' && rideDateTime > new Date() && minutesUntilRide <= 15 && minutesUntilRide > -30;
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {/* Live Tracking Section for Driver */}
                    {canStartTracking && (
                      <div className="mb-4">
                        {activeTrackingRide === ride._id ? (
                          <DriverLiveTracking 
                            rideId={ride._id}
                            onStopSharing={() => setActiveTrackingRide(null)}
                          />
                        ) : (
                          <button
                            onClick={() => setActiveTrackingRide(ride._id)}
