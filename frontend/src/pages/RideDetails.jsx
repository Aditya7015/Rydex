...
import { StarIcon as StarSolidIcon } from '@heroicons/react/24/solid';
import { FaCarSide, FaSnowflake, FaSmoking, FaDog, FaMusic, FaComments, FaLeaf, FaLocationArrow } from 'react-icons/fa';
import { useAuth } from '../context/AuthContext';
...
export default function RideDetails() {
  ...
  const [isDriverSharing, setIsDriverSharing] = useState(false);
  const [minutesUntilRide, setMinutesUntilRide] = useState(0);
  const isDriver = user && ride?.driverId?._id === user.id;
  const hasAcceptedBooking = !isDriver && ride?.bookedPassengers?.some(
    (b) => b.status === 'accepted' && String(b.passengerId?._id || b.passengerId) === String(user?.id)
  );
  ...
  useEffect(() => {
    if (ride) {
      ...
      if (rideTime) {
        const now = new Date();
        const minutes = (rideTime - now) / (1000 * 60);
        setMinutesUntilRide(minutes);
      }
    }
  }, [ride]);

  useEffect(() => {
    if (ride && hasAcceptedBooking) {
      const checkTrackingStatus = async () => {
        try {
          const { data } = await axios.get(`/rides/${ride._id}/tracking-status`);
          setIsDriverSharing(data.isSharing);
        } catch (error) {
  ...
  const canBook = !isDriver && ride.status === 'active' && ride.availableSeatsCount > 0;
  const canStartTracking = isDriver && minutesUntilRide <= 15 && minutesUntilRide > -30;
  const rideDateTime = getRideDateTime(ride);
  const now = new Date();
  const isUpcoming = rideDateTime ? rideDateTime > now : false;
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
              ...
              {/* Driver Live Tracking Button */}
              {isDriver && canStartTracking && !showTracking && isUpcoming && (
                <button
                  onClick={() => setShowTracking(true)}
                  className="w-full bg-green-600 text-white py-2 rounded-xl font-medium hover:bg-green-700 transition-all flex items-center justify-center gap-2 mt-4"
                >
              ...
              {isDriver && showTracking && (
                <div className="mt-4">
                  <DriverLiveTracking 
                    rideId={ride._id}
                    onStopSharing={() => setShowTracking(false)}
                  />
                </div>
              )}

              {/* Passenger Live Tracking Button */}
              {!isDriver && hasAcceptedBooking && isDriverSharing && !showTracking && isUpcoming && (
                <button
                  onClick={() => setShowTracking(true)}
                  className="w-full bg-blue-600 text-white py-2 rounded-xl font-medium hover:bg-blue-700 transition-all flex items-center justify-center gap-2 mt-4"
                >
              ...
              {/* Passenger Live Tracking Component */}
              {!isDriver && showTracking && (
                <div className="mt-4">
                  <PassengerLiveTracking 
                    rideId={ride._id}
                    driverName={ride.driverId.name}
                    onTrackingEnd={() => setShowTracking(false)}
                  />
                </div>
              )}

              {/* Message and Contact Buttons */}
              {user && !isDriver && (
                <div className="mt-4 space-y-2">
                  <button
                    onClick={() => navigate(`/chat/${ride._id}/${ride.driverId._id}`)}
